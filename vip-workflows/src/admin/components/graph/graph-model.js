...
export function insertStageOnEdge( stages, from, to, options = {} ) {
	...
	if ( isAgentOutcome( outcome ) ) {
		...
		// label is left to be regenerated for the new destination. Unlike a
		// rewire, the original stays behind, so the copy's assignment takes a
		// slot of its own.
		const existing = findTransition( withStage, from, to );
		const { to: _drop, label: _label, ...fields } = existing || {};
		const rerouted = routeOutcome(
			withStage,
			from,
			outcome,
			key,
