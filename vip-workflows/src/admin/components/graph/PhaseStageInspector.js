 ...
 *
 * Adding one is a button rather than a canvas gesture because there is only
 * ever one answer — a phase sequence may draw exactly the hand-offs it owes, so
 * nothing is being chosen. The drag still works and still says the same thing;
 * this is the way to it that doesn't have to be guessed at.
 *
 * @package
 */
	...
	return (
		...
		>
			<Stack direction="column" gap="lg" align="stretch">
				...
				>
					{ missing.length > 0
						? /* Not "select the connection": there isn't one to
						     select, which is the whole complaint. A panel that
						     names a missing thing as the way to fix itself is
						     the dead end this offer exists to end. */
						  __(
								'Phases are fixed. This one owes a hand-off that has not been drawn, and content cannot leave it until one is.',
								'vip-workflows'
						  )
						: __(
				...
				{ missing.map( ( { to } ) => (
					...
					>
						{ sprintf(
							...
							__( 'Add the hand-off to “%s”', 'vip-workflows' ),
