...
export function KanbanBoard() {
	...
	const fetchData = useCallback( async () => {
		try {
			...
			setError(
				err.message || __( 'Could not load the board. Reload the page to try again.', 'vip-workflows' )
			);
		} finally {
			setLoading( false );
		}
	...
	const handleDragEnd = async ( event ) => {
		...
		if ( ! destColumn ) {
			...
			createErrorNotice( __( 'Could not move the card.', 'vip-workflows' ), {
				type: 'snackbar',
			} );
			return;
		}
		...
		try {
			...
			let errorMessage = __( 'Could not move the card.', 'vip-workflows' );
			if ( err.message ) {
