 ...
 * VIP Workflow extensions. Uses ability_class so Core instantiates our Ability subclass
 ...
 * @package VIPWorkflow
 * @see     WP_Ability
 * @see     VIPWorkflow\Abilities\Ability
 */
...
/**
 * Register a VIP Workflow ability.
 *
 ...
 *
 * @param string $name Namespaced ability identifier (e.g. 'vip-workflow/web-researcher').
 ...
 */
function vip_workflow_register_ability( string $name, array $args ): ?\WP_Ability {
 ...
 */
function vip_workflow_get_ability( string $name ): ?\WP_Ability {
 ...
 * @return \WP_Ability[]
 */
function vip_workflow_get_abilities(): array {
 ...
 * @return bool
 */
function vip_workflow_has_ability( string $name ): bool {
 ...
 */
function vip_workflow_execute_ability( string $name, array $input = array() ) {
	$ability = vip_workflow_get_ability( $name );

	if ( ! $ability ) {
		throw new \Exception(
			/* translators: %s: ability name. */
			sprintf( esc_html__( 'Ability "%s" not found.', 'vip-workflow' ), esc_html( $name ) )
		);
	}
 ...
 */
function vip_workflow_unregister_ability( string $name ): ?\WP_Ability {
