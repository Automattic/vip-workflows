...
Every extension lives in a sibling directory at the repo root and is a standalone WordPress plugin with `Requires Plugins: vip-workflows` in its header. They install and activate independently.
...
Extension plugins loaded via Composer classmap are not constrained by this rule, but any class placed inside `vip-workflows/includes/` must follow it or the autoloader will fail to locate it on Linux/case-sensitive filesystems.

## Deep reference: skill docs

The `vip-workflows/skills/` directory contains copy-pasteable SKILL docs written for AI agents. Humans can read them too; they are short, task-focused, and include working examples.
...
Full details, including the Workflows submenu ordering (core pages, then third-party pages in the Integrations group), are in [`vip-workflows/docs/PLUGIN-INTEGRATION.md`](../../vip-workflows/docs/PLUGIN-INTEGRATION.md).
...
- **Sequences**: ship a workflow as JSON and create it without the graph editor. Author the config from the [`create-vip-workflows-sequence` skill](../../vip-workflows/skills/create-sequence/SKILL.md) or the canonical template [`editorial-basic.json`](../../vip-workflows/includes/sequences/templates/editorial-basic.json), and see [`docs/examples/`](../examples/) for the full pipeline in both shapes. An extension ships a sequence by importing the WRAPPED envelope (`POST /sequences/import`, then activate) or creating the FLAT shape (`POST /sequences`); import creates a draft and regenerates assignment meta keys. Endpoints and permissions are in [`quick-reference.md`](../reference/quick-reference.md#key-rest-endpoints).
...
Your plugin owns its own test suite. Core's bootstrap loads only `vip-workflows.php` and never reads `active_plugins`, so the core suite cannot see your plugin even when wp-env has it active — copy [`workflow-parsely/tests/bootstrap.php`](../../workflow-parsely/tests/bootstrap.php), which loads its dependencies in order and says so in its own docblock.
...
Coverage lives in `vip-workflows/tests/phpunit/Unit/ExtensionAbilityPermissionTest.php` and, for the Parse.ly bridge, `workflow-parsely/tests/AbilityPermissionTest.php`. Add new abilities to whichever fits.
