# VIP Workflow documentation

## Start here

- [AI context index](AI_CONTEXT.md) — architecture and subsystem reference map.
- [Local development](local-development.md) — install, build, and run the project.
- [Testing](../vip-workflows/docs/TESTING.md) — unit, integration, and end-to-end test environments.
- [Extending VIP Workflow](guides/extending-vip-workflows.md) — supported extension points and examples.

## Reference

- [Architecture](reference/architecture.md)
- [File structure](reference/file-structure.md)
- [Database schema](reference/database-schema.md)
- [Data flows](reference/data-flows.md)
- [Code patterns](reference/code-patterns.md)
- [Admin UI](reference/admin-ui.md)
- [Extension points](reference/extension-points.md)
- [AI models and providers](reference/ai-supported-models.md)
- [Quick reference](reference/quick-reference.md)
- [Stage-agent verdict threat model](reference/stage-agent-verdict-threat-model.md)

## Guides

- [Action standard](guides/action-standard.md)
- [Modal standard](guides/modal-standard.md)
- [Settings standard](guides/settings-standard.md)
- [WPDS usage audit patterns](guides/wpds-usage-audit-patterns.md)
- [Security review rubric](guides/security-sweep-rubric.md)

## Feature references and demos

- [Shipped feature specifications](specs/README.md)
- [Multimedia sequence demo](demos/multimedia-sequence.json)
- [Plugin integration](../vip-workflows/docs/PLUGIN-INTEGRATION.md)
